...
contract Vintner is ERC721Enumerable, Ownable, Pausable {
     ...
     */
    constructor(VintageWine _vintageWine, address _vintnerTypeOracleAddress, string memory _BASE_URI) ERC721("VintageWine Game Vintners", "VINTAGEWINE-GAME-VINTNER") {
    ...
    function mintingStartedVINTAGEWINE() public view returns (bool) {
        return startTimeVINTAGEWINE != 0 && block.timestamp >= startTimeVINTAGEWINE;
    }
    ...
    function setvintnerTypeOracleAddress(address _vintnerTypeOracleAddress) external onlyOwner {
        vintnerTypeOracleAddress = _vintnerTypeOracleAddress;
    }
    ...
    function _createVintner(address to, uint256 tokenId) internal {
        require (vintnersMinted <= NUM_GEN0_VINTNERS + NUM_GEN1_VINTNERS, "cannot mint anymore vintners");
    ...
    function setVintnerType(uint256 tokenId, uint256 vintnerType) external {
        require(_msgSender() == vintnerTypeOracleAddress, "msgsender does not have permission");
        require(tokenTypes[tokenId] == 0, "that token's type has already been set");
        require(vintnerType == VINTNER_TYPE || vintnerType == MASTER_VINTNER_TYPE, "invalid vintner type");
     ...
     */
    function mintPromotional(uint256 qty, uint256 vintnerType, address target) external onlyOwner {
        require (qty > 0, "quantity must be greater than 0");
        require ((vintnersMintedPromotional + qty) <= PROMOTIONAL_VINTNERS, "you can't mint that many right now");
        require(vintnerType == VINTNER_TYPE || vintnerType == MASTER_VINTNER_TYPE, "invalid vintner type");

        for (uint256 i = 0; i < qty; i++) {
            vintnersMintedPromotional += 1;
            require(tokenTypes[vintnersMintedPromotional] == 0, "that token's type has already been set");
    ...
    function mintWhitelist(bytes32[] calldata _merkleProof, uint256 qty) external payable whenNotPaused {
        ...
        require((vintnersMintedWhitelist + qty) <= WHITELIST_VINTNERS, "you can't mint that many right now");
        require((whitelistClaimed[_msgSender()] + qty) <= MAXIMUM_MINTS_PER_WHITELIST_ADDRESS, "this address can't mint any more whitelist vintners");
    ...
    function mintVintnerWithAVAX(uint256 qty) external payable whenNotPaused {
        ...
        require ((vintnersMintedWithAVAX + qty) <= (NUM_GEN0_VINTNERS - vintnersMintedWhitelist - PROMOTIONAL_VINTNERS), "you can't mint that many right now");
    ...
    function mintVintnerWithVINTAGEWINE(uint256 qty) external whenNotPaused {
        ...
        require (qty > 0 && qty <= 10, "quantity must be between 1 and 10");
        require ((vintnersMintedWithVINTAGEWINE + qty) <= NUM_GEN1_VINTNERS, "you can't mint that many right now");

        // calculate transaction costs
        uint256 transactionCostVINTAGEWINE = currentVINTAGEWINEMintCost * qty;
        require (vintageWine.balanceOf(_msgSender()) >= transactionCostVINTAGEWINE, "not enough VINTAGEWINE");
        ...
        if(vintnersMintedWithVINTAGEWINE <= VINTNERS_PER_VINTAGEWINE_MINT_LEVEL && vintnersMintedWithVINTAGEWINE + qty > VINTNERS_PER_VINTAGEWINE_MINT_LEVEL) {
    ...
    function batchedVintnersOfOwner(address _owner, uint256 _offset, uint256 _maxSize) public view returns (VintnerInfo[] memory) {
