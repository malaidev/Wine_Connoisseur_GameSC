...
contract VintageWineFountain is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public initialLPValue; // Initial number of LP tokens of USDC-VintageWine
    ...
    function getRewardPerSecond() public view returns (uint256) {
        ...
        return
            rewardPerSecondCalculation(
                lpSupply,
                initialLPValue,
                initialLPAvax,
                rewardMultiplier,
                rewardMultiplierDivision,
                baseRate
            );
    }
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTimestamp && lpSupply != 0) {
            ...
            accRewardToken = accRewardToken.add(
    ...
    function setRewardMultiplierDivision(uint256 _rewardMultiplierDivision)
        external
        onlyOwner
    {
    ...
    function rewardPerSecondCalculation(
        uint256 _lpSupply,
        uint256 _initialLPValue,
        uint256 _initialLPAvax,
        uint256 _rewardMultiplier,
        uint256 _rewardMultiplierDivision,
        uint256 _baseRate
        ...
        if (
            ...
            _initialLPAvax == 0 ||
            _rewardMultiplier == 0 ||
            _rewardMultiplierDivision == 0
        ) {
            return _baseRate;
        }
        return
            _baseRate +
            _rewardMultiplier.mul(_lpSupply).mul(_initialLPAvax).div(
                _initialLPValue.mul(_rewardMultiplierDivision).mul(86400)
            );
    }
    ...
    function updatePool() public whenNotPaused {
        ...
        accRewardTokenPerShare = accRewardTokenPerShare.add(
            multiplier.mul(getRewardPerSecond()).mul(1e12).div(lpSupply)
        );
        lastRewardTimestamp = block.timestamp;
    }
    ...
    function withdraw(uint256 _amount) public whenNotPaused nonReentrant {
        ...
        // Harvest Reward
        uint256 pending = user.amount.mul(accRewardTokenPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeTokenTransfer(msg.sender, pending);
    ...
    function setManualRewardPerSecond(uint256 _manualRewardPerSecond)
        external
        onlyOwner
    {
